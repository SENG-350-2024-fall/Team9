...
| **Steps**                                   | 1. The patient opens Mister Ed and selects the "Waitlist View" option.<br> 2. The patient observes the current waitlist level of releveant EDs on the UI (e.g., 35%, 60%, ...).<br> 3. The patient enter the conditions into the "Submit Issue" field. <br> 4. The patient is presented with an estimated earliest response tiem based on thier input and waitlist levels.  |
